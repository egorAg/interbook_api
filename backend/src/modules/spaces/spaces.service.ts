...
export class SpacesService {
    ...
    public async getSpaceById(id: number) {
        ...
        if ( !candidate ) {
            throw new HttpException(`Can't find space with id: ${id}`, HttpStatus.BAD_REQUEST)
        }
    ...
    public async createInviteToken(invitor: number, spaceId: number, userId?: number) {
        ...
        if ( space.adminId !== invitor ) {
            throw new HttpException(`You are not an admin`, HttpStatus.METHOD_NOT_ALLOWED)
        }
        ...
        await this.keyStorage.set(KeyStorageKeys.SPACE_INVITE(inviteToken), token, 24 * 60 * 60)

        EventBus.pub(EventsKeys.SPACE_INVITE.produce({
            user: from,
            space,
            token: inviteToken
        }))
    ...
    public async acceptInvite(inviteToken: string, userId: number) {
        ...
        if ( !token ) {
            throw new HttpException(`Can't find provided token`, HttpStatus.BAD_REQUEST)
        }

        const tokenPayload = await this.cryptoService.decodeSpaceToken(token);

        if ( !tokenPayload.spaceId ) {
            throw new HttpException(`Can't parse spaceId from token`, HttpStatus.BAD_REQUEST)
        }
        ...
        if ( spaceUsers.includes(user.id) ) {
            throw new HttpException(`You already in this space`, HttpStatus.BAD_REQUEST)
        }
